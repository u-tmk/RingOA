...
PARSE_SCRIPT="${PARSE_SCRIPT:-${SCRIPT_DIR}/parse_range_log.sh}"

DB_BITS="${DB_BITS:-25}"
NETWORKS="${NETWORKS:-LAN WAN}"
VALID_NETWORKS="LAN MAN WAN"

RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)_range}"
...
PARSED_TABLE="${PARSED_DIR}/range_table.tsv"

source "${SCRIPT_DIR}/benchmark_common.sh"

check_executable "${PARSE_SCRIPT}" "Range-query log parser"
initialize_benchmark_environment

run_benchmarks_for_network() {
  local network="$1"

  run_ringoa_3pc 17 "OblivRange VAF Preprocess" "oblivrange_vaf_preproc" "${network}"
  run_ringoa_3pc 18 "OblivRange VAF Online" "oblivrange_vaf_online" "${network}"
  run_ringoa_3pc 19 "OblivRange FSC VAF Preprocess" "oblivrange_fsc_vaf_preproc" "${network}"
  run_ringoa_3pc 20 "OblivRange FSC VAF Online" "oblivrange_fsc_vaf_online" "${network}"
  run_ringoa_3pc 21 "SotRange VAF Preprocess" "sotrange_vaf_preproc" "${network}"
  run_ringoa_3pc 22 "SotRange VAF Online" "sotrange_vaf_online" "${network}"
}
