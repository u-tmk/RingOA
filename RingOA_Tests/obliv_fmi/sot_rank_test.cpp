#include "sot_rank_test.h"

#include <random>

#include <cryptoTools/Common/TestCollection.h>

#include "RingOA/obliv_fmi/sot_rank.h"
...
const std::string kTestSotRankPath = kCurrentPath + "/data/test/obliv_fmi/sotrank/";
...
inline std::string MakePath(const std::string &name, uint64_t d, uint64_t k) {
    return kTestSotRankPath + name + "_d" + ringoa::ToString(d) + "_k" + ringoa::ToString(k);
}
...
using ringoa::fmi::SotRank;
using ringoa::fmi::SotRankKeys;
using ringoa::fmi::SotRankParameters;
using ringoa::fmi::SotRankPreprocessData;
...
void SotRank_DataGen_Test() {
    Logger::DebugLog(LOC, "SotRank_DataGen_Test...");

    const auto cases = BuildTestCases();

    for (const auto &tc : cases) {
        SotRankParameters params(tc.cfg, tc.share);
        ...
        std::array<Rep3ShareMat64, 3> db_sh       = sot_rank.GenerateDatabaseU64Share(fm);
    ...
    Logger::DebugLog(LOC, "SotRank_DataGen_Test - Passed");
}

void SotRank_Preprocess_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "SotRank_Preprocess_Test...");
    ...
    for (const auto &tc : cases) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                auto   preproc_data = sot_rank.Preprocess(chls, count);
                ...
                ProtocolIo::SaveToFile(MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id), preproc_data);
                Logger::DebugLog(LOC, "Saved SotRank preprocess data to file: " + MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id));
                ...
                SotRankPreprocessData loaded_data;
                ProtocolIo::LoadFromFile(MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id), loaded_data, params);
                Logger::DebugLog(LOC, "Loaded SotRank preprocess data from file: " + MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id));
            };
        };
    ...
    Logger::DebugLog(LOC, "SotRank_Preprocess_Test - Passed");
}

void SotRank_Online_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "SotRank_Online_Test...");
    ...
    for (const auto &tc : cases) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=, &result](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                ringoa::sharing::LoadRep3PreprocessDataFromFile(MakePath("rep3_prf_keys", d, k) + "_" + ToString(party_id), loaded_rep3_data);
                Logger::DebugLog(LOC, "Loaded Rep3 PRF preprocess data from file: " + MakePath("rep3_prf_keys", d, k) + "_" + ToString(party_id));
                SotRankPreprocessData preproc_data;
                ProtocolIo::LoadFromFile(MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id), preproc_data, params);
                Logger::DebugLog(LOC, "Loaded SotRank preprocess data from file: " + MakePath("sot_rank_preproc", d, k) + "_" + ToString(party_id));

                // Set PRF keys and SotRank keys
                ctx.Rss().SetPrfKeys(loaded_rep3_data.prf_key_self, loaded_rep3_data.prf_key_next);
                SotRankKeys keys = std::move(preproc_data).ExtractKeys();
                ...
                sot_rank.EvaluateRankCF(chls, keys.GetView(0), uv_prev, uv_next, db_sh, Rep3ShareView64(query_sh), position_sh, result_sh);
                sot_rank.EvaluateRankCFPair(chls, keys.GetView(1), keys.GetView(2), uv_prev, uv_next, db_sh, Rep3ShareView64(query_sh), position_vec_sh, result_vec_sh);
                Logger::DebugLog(LOC, "[P" + ToString(party_id) + "] sent: " + ToString(ctx.StopCommStats(chls)) + " bytes");
        ...
        if (result != expected_result) {
            throw osuCrypto::UnitTestFail(
                "SotRank_Online_Test failed: result = " + ToString(result) +
                ", expected = " + ToString(expected_result));
        }
    }

    Logger::DebugLog(LOC, "SotRank_Online_Test - Passed");
}
