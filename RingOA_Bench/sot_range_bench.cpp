...
inline std::string MakePath(const std::string &name, uint64_t d, uint64_t k) {
    return bench_ringoa::kBenchSotRangePath + name + "_d" + ringoa::ToString(d) + "_k" + ringoa::ToString(k);
}
...
void SotRange_VAF_Preprocess_Bench(const osuCrypto::CLP &cmd) {
    if (IsHelpRequested(cmd)) {
        PrintBenchmarkHelp("SotRange Benchmark");
        return;
    }
    const auto opts = SelectBenchOptions(cmd);

    Logger::InfoLog(LOC, "SotRange VAF Preprocess Benchmark started (repeat=" + ToString(opts.repeat) + ")");
    ...
    auto MakeTask = [&](int p) {
        ...
        return [=](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
            ...
            ringoa::sharing::SaveRep3PreprocessDataToFile(kBenchSotRangePath + "rep3_prf_keys_" + ToString(p), rep3_data);
    ...
    Logger::InfoLog(LOC, "SotRange VAF Preprocess Benchmark completed");
    Logger::ExportLogListAndClear(
        MakeLogBasePath(opts, kLogSotRangePath, "sotrange_vaf_preproc", opts.party_id),
...
void SotRange_VAF_Online_Bench(const osuCrypto::CLP &cmd) {
    ...
    Logger::InfoLog(LOC, "SotRange VAF Online Benchmark started (repeat=" + ToString(opts.repeat) +
    ...
    auto MakeTask = [&](int p) {
        ...
        return [=](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
            ...
            // ----- Timers -----
            const std::string timer_setup_name = "SotRange::VAF::OnlineSetUp" + ptag;
            const std::string timer_eval_name  = "SotRange::VAF::Eval" + ptag;
            ...
            SotRangePreprocessData              preproc_data;
            ringoa::sharing::LoadRep3PreprocessDataFromFile(kBenchSotRangePath + "rep3_prf_keys_" + ToString(p), loaded_rep3_data);
            ...
            for (uint64_t i = 0; i < opts.repeat; ++i) {
                ...
                ctx3p.Timers().Print(ctx3p.Timers().GetOrCreateTimer("SharedOT::LocalEval"), "d=" + ToString(d) + ",iter=" + ToString(i), ringoa::TimeUnit::MICROSECONDS);
    ...
    Logger::InfoLog(LOC, "SotRange VAF Online Benchmark completed");
    Logger::ExportLogListAndClear(
        MakeLogBasePath(opts, kLogSotRangePath, "sotrange_vaf_online", opts.party_id),
        opts.enable_log_timestamp);
}
