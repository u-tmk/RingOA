...
public:
    ...
    BinarySharing2P(const ShareConfig &config);
    ...
    // Evaluate operations (xor, and)
    void EvaluateXor(const uint64_t x, const uint64_t y, uint64_t &z) const;
    template <size_t N>
    void EvaluateXor(const std::array<uint64_t, N> &x,
    ...
    void EvaluateAnd(const uint64_t      party_id,
                     osuCrypto::Channel &chl,
                     const uint64_t      x,
                     const uint64_t      y,
                     uint64_t           &z);
...
private:
    ...
    void ReconstCore(const uint64_t      party_id,
                     ...
                     std::span<uint64_t> x) const;
    void EvaluateXorCore(std::span<const uint64_t> x,
                         std::span<const uint64_t> y,
                         std::span<uint64_t>       z) const;
    void EvaluateAndCore(const uint64_t            party_id,
                         osuCrypto::Channel       &chl,
